...
class GRPOTrainer(BaseTrainer):
                    ...
                    disable=not self.accelerator.is_local_main_process,
                        ...
                        with self.autocast():
                            ...
                            if self.enable_kl_penalty:
                                if self.training_args.kl_type == 'z-based':
                        ...
                        if self.enable_kl_penalty:
                            with self.autocast(), torch.no_grad(), self.adapter.use_ref_parameters():
                                ...
                                elif self.training_args.kl_type == 'z-based':
                                   
                                    ref_cond_out = self.adapter.forward(
                                        batch,
                                        timestep_index=timestep_index,
                                        compute_log_prob=False,
                                        return_kwargs=['noise_pred'],
                                    )
                                    v_cond = ref_cond_out.noise_pred

                                    import copy
                                    
                                    null_batch = copy.copy(batch) 
                                    
                                  
                                    if hasattr(null_batch, 'cap_feats'):
                                      
                                        if isinstance(null_batch.cap_feats, list):
                                            null_batch.cap_feats = [torch.zeros_like(c) for c in null_batch.cap_feats]
                                        else:
                                            null_batch.cap_feats = torch.zeros_like(null_batch.cap_feats)
                                    
                                
                                    ref_uncond_out = self.adapter.forward(
                                        null_batch,
                                        timestep_index=timestep_index,
                                        compute_log_prob=False,
                                        return_kwargs=['noise_pred'],
                                    )
                                    v_uncond = ref_uncond_out.noise_pred

                                    guidance_scale = getattr(self.training_args, "dmd_cfg", 3.0)
                                    v_target = v_uncond + guidance_scale * (v_cond - v_uncond)

                                   
                                    dmd_loss = torch.nn.functional.mse_loss(
                                        output.noise_pred.float(), 
                                        v_target.detach().float(), 
                                        reduction="mean"
                                    )
                                    
                                   
                                    kl_div = torch.clamp(dmd_loss, max=0.1)
    ...
    def evaluate(self) -> None:
        # print(any("lora" in n for n, _ in self.adapter.transformer.named_parameters()))
